            ...
            <div class="card-header align-items-center py-5 gap-2 gap-md-5">
                ...
                <div class="card-toolbar flex-row-fluid justify-content-end gap-5">
                    <div class="input-group w-250px">
                        <input class="form-control form-control-solid rounded rounded-end-0" placeholder="Tanggal Komentar"
                        ...
                        @foreach ($comments as $comment)
                            <tr>
                                ...
                                <td data-kt-ecommerce-order-filter="order_id">
                                    <a href="#"
                                        class="text-gray-800 text-hover-primary fw-bold">{{ $comment->id }}</a>
                                </td>
                                <td>
                                ...
                                <td class="text-end">
                                    <a href="#"
                                        class="text-gray-600 fw-bold text-hover-primary">{{ $comment->news->title }}</a>
                                    <br>
                                    <span
                                        class="text-muted fw-bold">{{ $comment->parent_id ? 'Reply ID: ' . $comment->parent_id : 'Comment' }}</span>
                                </td>
                                <td class="text-end">
                                    ...
                                    <br>
                                    <span
                                        class="text-muted fw-bold">{{ $comment->news->created_at->format('d/m/Y') }}</span>
                                </td>
                                <td class="text-end">
                                    <form style="display: inline-block;"
                                        action="{{ route('back.news.comment.spam', $comment->id) }}" method="POST">
                                        @csrf
                                        <button type="submit" class="btn btn-icon btn-light-youtube me-2"
                                            title="Jadikan Spam">
